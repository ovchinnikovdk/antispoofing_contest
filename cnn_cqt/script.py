...
if __name__ == '__main__':
    model = CQTChromogram(input_shape=(24, 130))
    model.load_state_dict(torch.load('cnn_cqt_model.dat'))
    model.cuda()
    model.eval()
    ...
    for protocol_id, protocol_row in tqdm.tqdm(list(eval_protocol.iterrows())):
        x = cqt_load(os.path.join(dataset_dir, protocol_row['path']))[None][None]
